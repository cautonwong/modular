...
edge_status_t edge_sys_bind_event_queue(edge_sys_t *sys,
                                        ...
                                        size_t capacity)
{
    if (sys == NULL || queue == NULL ||
        (subscriptions == NULL && capacity != 0u)) {
        return EDGE_EINVAL;
    }
...
{
    if (sys == NULL || (required_ids == NULL && required_count != 0u)) {
        return EDGE_EINVAL;
    }
    if (sys->state != EDGE_SYS_CONSTRUCTED) {
        return EDGE_ESTATE;
    }
    sys->required_ids = required_ids;
    sys->required_count = required_count;
    return EDGE_OK;
}
...
edge_status_t edge_sys_dispatch_events(edge_sys_t *sys)
{
    ...
    for (;;) {
        ...
        for (size_t i = 0u; i < sys->subscription_count; ++i) {
            ...
            if (sub->event_id == event.id && sub->app != NULL &&
                sub->app->on_event != NULL && !sub->app->failed) {
...
edge_status_t edge_sys_deinit(edge_sys_t *sys)
{
    ...
    if (sys->state == EDGE_SYS_FAILED || sys->state == EDGE_SYS_STOPPED) {
        for (size_t i = sys->app_count; i > 0u; --i) {
            edge_module_t *app = sys->apps[i - 1u];
            if (app->initialized && app->deinit != NULL) {
