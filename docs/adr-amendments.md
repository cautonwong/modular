...
| 2026-09-20 | §24 (D46/D49) | **Resolved a contradiction.** Vendor HAL and register-level code live in `soc/<soc>/`; `product/<name>/glue/` only *adapts* interfaces (D14) and never hosts a HAL. `docs/phase1.md`'s "wrap the vendor SDK in product glue" is superseded by this reading, and that line is reworded | #150 |
| 2026-09-20 | §24 (D3/D9) | **Resolved a contradiction.** The vector table belongs to `board/<board>/` (one per board); `soc/<soc>/` supplies reusable startup/link templates; `.github/arm/` keeps only the CI/QEMU variants until #125 moves them | #150 |
| 2026-09-20 | D84 | **Implemented.** `board_mps2_irq_attach(irq, cb, ctx)` plus `board_mps2_irq_dispatch(irq)`: one line carries several handlers, dispatched in registration order, with a fixed static table (no allocation, D21) and a bounded walk (D75). A line's vector entry is now the *dispatcher*, not a single handler, and the board's own timer consumer registers through the same public entry point so the built-in path exercises the mechanism. Priority policy split: `soc/<soc>/` owns the encoding and the legal range, `board/` owns the per-board assignment | #150 |
| 2026-09-19 | D55 | **Tightened and made drivable.** The table is now an X-macro list (`EDGE_MODULE_IDS`) with one line per ID: the constant, the `0xNN00` assertion and the new per-layer block assertion are generated from that line, replacing N*(N-1)/2 hand-written pairwise assertions. Segments are allocated in **blocks per owning layer** (`0x10xx-0x2Fxx` app, `0x30xx-0x3Fxx` infra, `0x40xx-0x4Fxx` sys, `0x50xx-0x5Fxx` board/soc), and the owning layer is declared in the table because a module such as DLMS or MODBUS has no directory to infer it from. `check_module_ids.py` additionally rejects an out-of-block value and a layer without a block | #115 |
