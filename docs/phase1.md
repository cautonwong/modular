...
- [x] **#47 `app/modbus_slave`**：Modbus RTU 从机（0x01/0x03/0x05/0x06/0x10 + 异常响应 + CRC16），消费者定义 store/transport 端口；`product/meter_gateway_host` 在同一 runner 上同时跑 `dlt645` + `modbus_slave`（双协议共存冒烟）。
- [ ] #48 UART/GPIO 端口契约
