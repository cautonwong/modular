...
| Platform abstraction | partial | `edge/pal.h` + tested `pal/host`; `pal/os` (`edge_os_port_t` yield/sleep) with `edge_os_idle_hook` bridging it to the sys idle hook. cortex-m PAL and the FreeRTOS task host remain. |
