...
| Platform abstraction | partial | `edge/pal.h` + tested `pal/host`; `pal/os` (`edge_os_port_t` yield/sleep) with `edge_os_idle_hook`; a neutral `pal/rtos` host contract with a FreeRTOS implementation running the capsule as one task (`product/meter_mps2_freertos`). A cortex-m-bare PAL remains. |
