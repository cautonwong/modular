...
| D50 | RTOS neutrality: the framework calls no RTOS API, RTOS appears only in `product/main` and `pal/` | ✅ | No FreeRTOS or Zephyr header escapes `pal/rtos/*`: `pal_rtos_freertos.h` and `pal_rtos_zephyr.h` expose only `edge_*` types. `check_app_isolation.py` rejects `FreeRTOS.h` / `task.h` / `cmsis_os.h` / `zephyr/` in any app |
