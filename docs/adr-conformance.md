...
| HFI (sensorless start) | 🟡 | The angle tracker is ported and bit-exact: `foc_hfi_adjust_angle` (foc_math.c:766-778) - the `foc_hfi_max_err` truncation, the proportional term with the reference's double-literal gains (4000.0 / 10.0, which the reference's own TODO admits are unreviewed, so they are copied rather than tidied), the double integrator bounded by the fast speed estimate, and the wrap. Its differential harness links the reference's motor/foc_math.c unmodified, and the 36 golden vectors in test_foc_core are that run's own output. Still open: everything that actually excites the motor - the six-vector and V0V7 injection, the per-PWM-cycle current sampling, the flip counters, the `foc_hfi_amb_mode` gating and the handover to and from the observer. Fixed on the way there, and worth naming because it affected more than HFI: this port defined `M_PI` as a float literal where the reference uses glibc's double, so every angle wrap in both trackers landed about one float ULP away and accumulated over thousands of steps |
| Motor detection | 🟡 | The resistance measurement is ported from `mcpwm_foc_measure_resistance` (mcpwm_foc.c:1797-1908): the forced electrical phase (the reference's `m_phase_override`, which stops both the sensor and the observer branches being taken), CURRENT mode with id = 0, the |I|/200-per-millisecond ramp, the 50 ms lock wait, the accumulator the control loop fills (now in foc_core, gated on the control having run, read and cleared through separate calls because the reference uses them at different moments), the two averages divided in that order, both fault aborts, and the cleanup the `stop_after` flag gates. Its timing is the reference's: one millisecond of the procedure per accumulated millisecond, and the millisecond that reaches the target is the ramp's last. The inductance measurement is not ported, and cannot be until B2 is: `mcpwm_foc_measure_inductance` switches the configuration into FOC_SENSOR_MODE_HFI and drives the HFI voltages (:1927-1935), so it and COMM_DETECT_MOTOR_R_L that contains it wait on HFI. The flux-linkage and hall procedures are not read yet. All three answer EDGE_ENOTSUP rather than a plausible number, and the invented crawl and timing arithmetic this module used to carry for them is gone. Still open: everything except resistance, plus the COMM_DETECT_* wiring that drives it |
