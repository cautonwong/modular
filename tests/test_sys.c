...
{
    fake_app_t *a = (fake_app_t *)m->private_data;
    ++a->init_count;
    order[order_count++] = (int)m->module_id;
    return a->init_rc;
}
...
static edge_status_t fake_event(edge_module_t *m, const edge_event_t *e)
{
    fake_app_t *a = (fake_app_t *)m->private_data;
    ++a->event_count;
    assert_int_equal(e->id, EDGE_EVT_UART0_RX);
    return EDGE_OK;
}
...
static void make_app(fake_app_t *a, uint32_t id, uint32_t priority)
{
    *a = (fake_app_t){0};
    a->module = (edge_module_t){
        .module_id = id, .priority = priority,
        .init = fake_init, .poll = fake_poll, .on_event = fake_event,
        .power_off = fake_off, .deinit = fake_deinit, .private_data = a
    };
}
...
static void test_priority_and_tie_break(void **state)
{
    ...
    assert_int_equal(order[0], 20);
    assert_int_equal(order[1], 30);
    assert_int_equal(order[2], 10);
}
...
static void test_event_routing_and_shutdown(void **state)
{
    ...
    edge_sys_t sys;
    edge_event_t e = {.id = EDGE_EVT_UART0_RX};
...
static void test_init_failure_rolls_back(void **state)
{
    ...
    assert_int_equal(a.deinit_count, 1);
    assert_int_equal(a.initialized, 0u);
    assert_int_equal(b.failed, 1u);
}
...
int main(void)
{
    const struct CMUnitTest tests[] = {
        ...
        cmocka_unit_test(test_event_routing_and_shutdown),
