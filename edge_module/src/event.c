...
edge_status_t edge_event_queue_init(edge_event_queue_t *queue,
                                    ...
                                    uint32_t capacity)
{
    if (queue == NULL || storage == NULL || capacity < 2u) return EDGE_EINVAL;
    queue->items = storage;
...
{
    if (queue == NULL || event == NULL || queue->items == NULL || queue->capacity < 2u) return EDGE_EINVAL;
...
edge_status_t edge_event_sink_push_isr(edge_event_sink_t *sink,
                                       const edge_event_t *event)
{
    if (sink == NULL || event == NULL || sink->queue == NULL) return EDGE_EINVAL;
    if (sink->guard != NULL && sink->guard->enter != NULL) sink->guard->enter(sink->guard->self);
    ...
    if (sink->guard != NULL && sink->guard->exit != NULL) sink->guard->exit(sink->guard->self);
    return rc;
}
...
edge_status_t edge_event_pop(edge_event_queue_t *queue, edge_event_t *event)
{
    ...
    if (tail == queue->head) return EDGE_ENOENT;
...
{
    if (queue == NULL || queue->items == NULL || queue->capacity < 2u) return 0u;
